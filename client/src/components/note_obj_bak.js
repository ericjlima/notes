...
class Notes extends Component {
  ...
  //TODO: think of how to get a child note since you can't use params aka branches variable below. Also and probably more importantly how do you deal with the same note names?
  componentDidMount() {
      ...
      try {
        //for(let i = 0; i < branches.length ; i++){
          const response = await axios.get(
            `${this.props.baseURL}/api/notes/children/${currentNoteData.id}`,
          );
          const children = response.data;
            console.log('children', children);
          children.map(e => {
            console.log('e', e.name);
             //this.state.childNotes.push(e.name);
            const addChild = this.state.childNotes;
  ...
  createNotesBranches = async (passedUpdateData) => {
    const branches = Object.entries(this.props.match.params);
    const parentData = [];
    let pid;
    ...
    for(let i = 0; i < branches.length ; i++){
      await axios.post(
        `${this.props.baseURL}/api/notes/${branches[i][1]}`
      );
      const obj = await axios.get(`${this.props.baseURL}/api/notes/${branches[i][1]}`);
      parentData.push(obj.data);
    }
    for(let i = 0; i < branches.length ; i++){
      if(i === 0) pid = 0; else pid = parentData[i-1][0].id;
      await axios.post(
        `${this.props.baseURL}/api/notes/update/${branches[i][1]}`,
      ...
      const updateNote = async (passedUpdateData) => {
        try {
          if (this.state.passEntered) {
            if(!!this.state.value){
              this.createNotesBranches(passedUpdateData);
            }
          }
        } catch (error) {
          this.setState({
